...
knitr::kable(summary(working_data %>% select(all_of(factor_list))), border_left = T, border_right = T)
knitr::kable(summary(working_data %>% select(c("hhsize", "iv009_mod", "sphus", "ch001_", "maxgrip", "adla", "br015_"))), border_left = T, border_right = T)
knitr::kable(summary(working_data %>% select(c("br010_mod", "chronic_mod", "isced1997_r", "country_mod", "eurod"))), border_left = T, border_right = T)
knitr::kable(summary(working_data %>% select(c("co007_", "age", "bmi"))), border_left = T, border_right = T)
```


The data above currently contains `NA`s which are filtered out while constructing the male and female models. 
...
Corresponding to the tables above, the boxplots below (in Figure 3) help visualise the relationship between BMI and some of the explanatory variables.
