...
Note: include para about work done by each participant
...
knitr::kable(summary(working_data %>% select(c("hhsize", "iv009_mod", "sphus", "ch001_", "maxgrip", "adla", "br015_"))))
knitr::kable(summary(working_data %>% select(c("br010_mod", "chronic_mod", "isced1997_r", "country_mod", "eurod"))))
knitr::kable(summary(working_data %>% select(c("co007_", "age", "bmi"))))
```


The data above contains NAs as we filter this out while constructing the male and female models. 



```{r Box-Plots-countries, fig.align='center', fig.height=4, fig.width=6, fig.cap="BMI varying across Countries"}
# Box plots to show how BMI varies across countries
boxplot(bmi ~ country_mod, data = working_data, xlab = "Countries", ylab = "BMI")
```


```{r Box-Plots-adla, fig.height=4, fig.width=6, fig.cap="BMI varying with Activities of Daily Living"}
# Box plot to show how BMI varies with Daily living activities
boxplot(bmi ~ adla, data = working_data, 
        xlab = "Activities of Daily Living index", ylab = "BMI")
```

From the boxplots in Figures 1 and 2, it is evident that although there isn't a major difference in the value of BMI in each country, there is a difference in the range of values. Austria and Germany seem to have a larger range of values than Netherlands and Sweden. It is also evident that there is not a large amount of variation in the BMI of those who have high mobility and those who have low mobility. However, it is worth noting that there is a larger range of values of BMI for those who have high mobility.  

The model assumptions of the initial model without separating the data by sex is displayed in Figure 2.

```{r Main-Model, fig.height=5.5, fig.width=6.5, fig.align='center', fig.cap="Checking Model Assumptions for Defined Model"}
...
```{r model-male-female, echo=FALSE}
...
We now look at the model applied to males and females separately. The model summaries for both models are shown below after model selection.
...
```{r present-boxplots-analysis, fig.height=8, fig.width=8, fig.cap="Possible risk factors of obsesity varying with BMI"}
...
```{r echo=FALSE, fig.height=8, fig.width=8, fig.cap="Possible risk factors of obsesity"}
